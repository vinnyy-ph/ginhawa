...
In `frontend/src/app/doctors/[id]/page.tsx`, around lines 283-289, update the rendering of `languagesSpoken` from displaying a raw string to rendering the array. Find the block:
```tsx
...
Replace the display of `{doctor.languagesSpoken}` with:
```tsx
...
The exact replacement depends on the surrounding JSX — keep the wrapper elements, just replace the string expression `{doctor.languagesSpoken}` with `{doctor.languagesSpoken.join(', ')}` and the condition `{doctor.languagesSpoken &&` with `{doctor.languagesSpoken && doctor.languagesSpoken.length > 0 &&`.
